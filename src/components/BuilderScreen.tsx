...
import type { CameraState, GridSpec, MaterialsEstimate, Orientation, Parameters, PlacedBrick, ToolKind, ViewMode } from "../domain/types";
...
const ThreeStack = lazy(() => import("./three/ThreeStack").then((m) => ({ default: m.ThreeStack })));
...
export function BuilderScreen(props: BuilderScreenProps) {
  const {
    ...
    orientation, setOrientation, viewMode, setViewMode, placeAt, addRow, copyPreviousRow,
    clearCurrentRow, lockRow, unlockRow, parameters, materials, camera, cameraZoom, cameraRotate,
      ...
      <div className="xl:grid xl:grid-cols-[320px_minmax(0,1fr)] xl:gap-4 xl:items-start">
        ...
        <section className="space-y-3">
          <div className="rounded-[26px] border-2 border-[#3D2B1F]/10 bg-[#F5E6C8] p-2 shadow-md shadow-[#3D2B1F]/10 min-h-[620px] xl:min-h-[min(70dvh,900px)]">
            {viewMode === "2d"
              ? <PlanGrid grid={grid} bricks={currentBricks.filter((brick) => isInsideGrid(brick, grid))} activeTool={activeTool} orientation={orientation} placeAt={placeAt} t={t} />
              : (
                <ErrorBoundary fallback={<CanvasFallback>{t("aria3d")}</CanvasFallback>}>
                  <Suspense fallback={<CanvasFallback>{t("view3d")}…</CanvasFallback>}>
                    <ThreeStack grid={grid} bricks={visibleBricks} currentRow={currentRow} placeAt={placeAt} t={t} camera={camera} activeTool={activeTool} orientation={orientation} />
                  </Suspense>
                </ErrorBoundary>
              )}
          </div>
...
function MobileRowRail({ rowCount, currentRow, lockedRows, setCurrentRow, t, addRow }: { rowCount: number; currentRow: number; lockedRows: number[]; setCurrentRow: (row: number) => void; t: Translate; addRow: () => void }) {
  return (
...
function Toolbox({ t, activeTool, setActiveTool, orientation, setOrientation, viewMode, setViewMode }: { t: Translate; activeTool: ToolKind; setActiveTool: (tool: ToolKind) => void; orientation: Orientation; setOrientation: (orientation: Orientation) => void; viewMode: ViewMode; setViewMode: (mode: ViewMode) => void }) {
  return (
...
function MobileActionBar({ t, currentRow, lockedRows, copyPreviousRow, clearCurrentRow, lockRow, unlockRow }: { t: Translate; currentRow: number; lockedRows: number[]; copyPreviousRow: () => void; clearCurrentRow: () => void; lockRow: () => void; unlockRow: () => void }) {
