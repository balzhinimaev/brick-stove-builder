  ...
  return (
    <div className="overflow-hidden rounded-[24px] border-2 border-[#3D2B1F]/10 bg-[#FFF7E8]">
      <svg width="100%" height="220" viewBox="0 0 192 220">
        <title>Stove side silhouette</title>
        ...
        <line
          ...
          strokeDasharray="6 5"
        />
        <text x="22" y={ceilingY - 6} fill={COLORS.sageDark} fontSize="10" fontWeight="800">
          {t("ceilingLimit")}
        </text>
        <rect
          x={x - 12}
        ...
        {Array.from({ length: Math.max(8, (lockedRows.length ? Math.max(...lockedRows) : 0) + 2) }).map((_, index) => {
          ...
          return (
            <rect
              ...
              height="10"
              rx="3"
              fill={locked ? COLORS.brickRed : COLORS.creamDark}
              stroke={locked ? COLORS.mortar : "rgba(61,43,31,0.15)"}
              strokeWidth="1"
              opacity={locked ? 1 : 0.42}
            />
          );
        })}
