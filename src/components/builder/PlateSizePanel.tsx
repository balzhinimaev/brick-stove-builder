...
export function PlateSizePanel({
  t,
  plateSpec,
  setPlateSize
}: {
  ...
  return (
    <div className="rounded-[22px] border border-[#3D2B1F]/10 bg-[#FFF7E8] p-2">
      <div className="mb-2 px-1 text-[11px] font-black uppercase tracking-wide text-[#3D2B1F]/55">
        {t("plateSizeTitle")}
      </div>
        ...
        {PLATE_PRESETS.map(([l, w]) => {
          ...
          return (
            <button
              type="button"
              key={`${l}x${w}`}
              onClick={() => setPlateSize(l, w, thicknessMm, flush)}
              aria-pressed={active}
              className={`min-h-9 rounded-2xl px-1 text-[11px] font-black ${active ? "bg-[#3D2B1F] text-[#F5E6C8]" : "bg-[#F5E6C8] text-[#3D2B1F]"}`}
            >
              {l}×{w}
            </button>
          );
      ...
      <div className="mt-2 px-1">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <button
            type="button"
            onClick={() => setPlateSize(lengthMm, widthMm, thicknessMm, false)}
            aria-pressed={!flush}
            className={`min-h-9 rounded-2xl px-1 text-[11px] font-black ${!flush ? "bg-[#3D2B1F] text-[#F5E6C8]" : "bg-[#F5E6C8] text-[#3D2B1F]"}`}
          >
            {t("plateOnTop")}
          </button>
          <button
            type="button"
            onClick={() => setPlateSize(lengthMm, widthMm, thicknessMm, true)}
            aria-pressed={flush}
            className={`min-h-9 rounded-2xl px-1 text-[11px] font-black ${flush ? "bg-[#3D2B1F] text-[#F5E6C8]" : "bg-[#F5E6C8] text-[#3D2B1F]"}`}
          >
            {t("plateFlush")}
