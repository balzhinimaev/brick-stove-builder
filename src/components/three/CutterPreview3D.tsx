...
export default function CutterPreview3D({ spec }: { spec: CustomBrickSpec }) {
  ...
  const solids = brickSolids(brick);
  ...
  const boxMesh = (box: BrickBox, height: number, y0: number, color: string, key: string | number, opacity = 1) => (
    <mesh
      key={key}
      position={[(box.x1 + box.x2) / 2 - cx, y0 + height / 2, (box.y1 + box.y2) / 2 - cz]}
      castShadow
      receiveShadow
    >
  ...
  return (
    <Canvas dpr={[1, 2]} camera={{ position: [1.7, 1.6, 2.1], fov: 34 }}>
      ...
      {notch
        ? label(
            `${notchLenMm}×${notchWidMm}×${notchDepthMm(brick)} мм`,
            (notch.x1 + notch.x2) / 2 - cx,
            (notch.y1 + notch.y2) / 2 - cz,
            DIM,
            0.1
          )
        : null}
