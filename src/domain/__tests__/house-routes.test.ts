...
describe.each(["R1", "R2"] as const)("%s routes", (revision) => {
  const project = makeHouseRussianStove(revision);
  function volume(gates: Record<string, number>, extra: PlacedBrick[] = []) {
    const stock = Object.values(project.rows)
      .flat()
  ...
  it("seals mortar beds but never fills a removed physical arch part", () => {
    const parts = Object.values(project.rows)
      .flat()
