...
describe("plateSeatZ — фактическая высота поверхности опоры", () => {
  it("полка «под плиту» 14 мм → посадка 51, верх заподлицо с рядом", () => {
    const seat = plateSeatZ([rebate("r", 0, 0, 14)], flushPlate("p", 0.5, 0.5));
    expect(seat).toBe(51);
  });

  it("глубокая четверть 32.5 остаётся на высоте 32.5", () => {
    const seat = plateSeatZ([rebate("r", 0, 0, 32.5)], flushPlate("p", 0.5, 0.5));
    expect(seat).toBe(32.5);
  });

  it("учитывает полнотелую часть под следом, а не только полки", () => {
    const seat = plateSeatZ([rebate("a", 0, 0, 32.5), rebate("b", 0, 1, 14)], flushPlate("p", 0.5, 0.5, 125, 250));
    expect(seat).toBe(65);
  });

  it("под следом пусто → высота опоры 0 (для flush-посадки недостаточно)", () => {
  ...
  it("накладные и другой колосник — не опора", () => {
    const damper: PlacedBrick = {
      ...
      kind: "damper",
      orientation: "h",
      custom: { name: "", w: 2, h: 1, notch: null, thicknessMm: 20 }
    };
...
describe("planPlacement — посадка штампуется при установке", () => {
  it("flush-плита над мелкой полкой: автоподрез до толщины плиты", () => {
    const rows = { 2: [rebate("r", 0, 0, 10)] };
    const plan = planPlacement(rows, 2, [flushPlate("p", 0.5, 0.5)], grid);
    expect(plan.rows).not.toBeNull();
    // мелкая четверть углублена до толщины плиты — верх вровень с рядом
...
describe("planPlacement — клик плитой по плите = замена (пересадка)", () => {
  it("«поверх» → «в вырезы»: плита заменяется, а не дублируется", () => {
    ...
    const rows = planPlacement({ 2: [rebate("base", 1, 1, 14)] }, 2, [onTop], grid).rows!;
...
describe("колосник садится в вырезы, как плита", () => {
  ...
  it("plateSeatZ колосника учитывает тело кирпича и пустоту", () => {
    expect(plateSeatZ([rebate("r", 1, 1, 32.5)], grate("g", 1.5, 1, 250, 125))).toBe(65);
  ...
  it("кольцо не дублируется при замене колосника кликом", () => {
    ...
    for (let i = 0; i < ringAfter.length; i++)
      for (let j = i + 1; j < ringAfter.length; j++) expect(overlaps(ringAfter[i], ringAfter[j])).toBe(false);
  });
  ...
  it("колосник заходит за паз в тело кирпича — вырез расширяется, не отказ", () => {
    ...
    expect(cut.kind).toBe("custom");
    // объединение старого паза (1.5..2) с зоной следа (1..2) → вырез 1..2, глубина остаётся 32.5
    expect(cut.custom?.notch).toEqual({ x1: 1, y1: 0, x2: 2, y2: 1 });
    expect(cut.custom?.notchDepthMm).toBe(32.5);
    const placed = plan.rows![2].find((b) => b.kind === "grate")!;
    expect(placed.custom?.seatZMm).toBe(43);
  });
...
describe("глубокая проверка: найденные недочёты", () => {
  ...
  it("copyRow не затирает ряд с колосником молча", async () => {
    ...
    const next = editorReducer(state, {
      type: "copyRow",
      bricks: [{ id: "c", row: 2, x: 5, y: 5, kind: "standard", orientation: "h" } as PlacedBrick]
    });
