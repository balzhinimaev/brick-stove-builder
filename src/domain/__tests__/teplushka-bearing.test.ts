...
it("has no masonry overlaps and a positive bearing-face chain to the foundation (not a load calculation)", () => {
  // Source structural steel is custom stock, deliberately included even though
  // the renderer's isMasonry predicate excludes it. Doors/plates are not supports.
  const stock = Object.values(RUSSIAN_STOVE.rows)
    .flat()
    .filter((b) => b.kind === "custom");
  const sourceSteel = stock.filter((b) => b.custom?.material === "steel");
  expect(sourceSteel).toHaveLength(7);
  ...
  for (const steel of sourceSteel)
    expect(world.some((s) => s.id === steel.id)).toBe(true);
  ...
  for (let i = 0; i < world.length; i++)
    for (let j = i + 1; j < world.length; j++) {
      ...
      if (
        a.bounds.some(
          ([lo, hi], k) =>
            Math.max(lo, b.bounds[k][0]) - Math.min(hi, b.bounds[k][1]) > 5.1,
        )
      )
        continue;
      if (
        a.bounds.every(
          ([lo, hi], k) =>
            Math.min(hi, b.bounds[k][1]) - Math.max(lo, b.bounds[k][0]) > 1e-5,
        )
      )
        expect(convexIntersects(a.shape, b.shape), `${a.id}/${b.id}`).toBe(
          false,
        );
      if (
  ...
  const founded = new Set(
    world.flatMap((s, i) => (s.bounds[2][0] <= 1e-6 ? [i] : [])),
  );
  const queue = [...founded];
  ...
  expect(
    world
      .filter((_, i) => !founded.has(i))
      .map((s) => ({ id: s.id, bounds: s.bounds })),
  ).toEqual([]);
}, 30000);
