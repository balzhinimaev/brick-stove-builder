...
const ru = {
  ...
  plateFlush: "В вырезы",
  plateFlushHint: "Плита сядет на полки вырезов под её контуром (глубже полка — глубже сядет); без полок верх остаётся заподлицо с рядом. Клик по стоящей плите — применить текущие размер и посадку.",
...
const en: Record<TranslationKey, string> = {
  ...
  plateFlushHint: "The plate rests on the cut ledges under its outline (deeper cut — deeper seat); with no ledges its top stays flush with the row. Click a placed plate to apply the current size and seating.",
...
const lt: Record<TranslationKey, string> = {
  ...
  plateFlushHint: "Plokštė atsiremia į išpjovų atbrailas po jos kontūru (gilesnė išpjova — giliau nusileidžia); be atbrailų viršus lieka lygus su eile. Spustelėkite pastatytą plokštę — pritaikyti dabartinį dydį ir įleidimą.",
